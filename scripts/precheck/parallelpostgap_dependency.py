...
def submit_a_chromosome(database_dir, tempdir, chrfile, memory, kstart, kmax, split_cluster):
	"""
	submit a job to run the pipeline for a chromosome, either to split by clusters or not
	Note:
	...
	* chrname = fnstart + '-chr' + chr
	...
	eqtl_db_part = " --hdf5 /nfs/production/panda/ensembl/funcgen/eqtl/GTEx.V6.88_38.cis.eqtls.h5 --sqlite yalan/GTEx.V6.88_38.cis.eqtls.h5.sqlite3"
...
def check_job_success(path, jobname):
	...
	if os.path.getsize(path + 'jobstatus.txt') == 0:
		...
		if 'Successfully completed' in outtext[-23]:
			...
			if 'job killed after reaching LSF run time limit' in outtext[-24] or int(outtext[-11].split()[-2]) > 10085 * 60:
				# unable to finish within LSF run time limit, which needs to be sped up
				success = float('nan')
				print(jobname + ': ' + outtext[-24])
			else:
				repeat = True
...
def track_job_error(path, jobname):
	...
	if not errtext:
		...
		return 'unknown error situation! just try again...'

def submit_a_cluster(database_dir, cluster_dir, cluster, chrfile, memory, kstart, kmax):
	"""
	submit a job to run the pipeline for a cluster
	Note:
	* type(memory/kstart/kmax): 'str'
	* cluster_dir = tempdir + chrname + '_clusters/'
	"""
	...
	* split_cluster: True / False
	...
	while not all(sumdf['success'] == True):
		for i,row in sumdf.loc[sumdf['success'] != True].iterrows():
			...
			if math.isnan(sumdf.loc[i, 'success']):
				print('chr' + chr + ' exceeded LSF run time limit, which calls for acceleration')
				sys.exit('reached LSF run time limit, please debug')
			elif sumdf.loc[i, 'success'] is True: # job finished successfully, record relevant parameters
			...
			else: # success = False, either running or to repeat
				if repeat:
	...
	* type(kstart/kmax): 'str'
	...
	while not all(sumdf['success'] == True):
		for i,row in sumdf.loc[sumdf['success'] != True].iterrows():
			...
			# just wait for few seconds, in case the stdout/stderr file has not been completed
			time.sleep(5)
			sumdf.at[i, 'success'], repeat, increase_mem = check_job_success(tempdir, job_cluster)
			
			if math.isnan(sumdf.loc[i, 'success']):
				print(cluster + ' exceeded LSF run time limit, which calls for acceleration')
				sys.exit('reached LSF run time limit, please debug')
			elif sumdf.loc[i, 'success'] is True: # job finished successfully, record relevant parameters
...
def merge_results(objectlist, tempdir, kstart, kmax, split_cluster):
	...
	* split_cluster: True / False
	...
	for n in objectlist:
		...
		# even if the file doesn't exist, it doesn't matter
		if n == objectlist[0]:
